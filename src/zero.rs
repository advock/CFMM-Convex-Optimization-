...
fn main() {
    ...
    // Constraint matrix G and vector h for Gx <= h
    let mut mat_g = MatBuild::new(MatType::General(9, n_vars));
    let mut vec_h = MatBuild::new(MatType::General(9, 1));

    // Constant product constraints (slightly relaxed)
    mat_g[(0, 0)] = reserves1[1];
    mat_g[(0, 1)] = -reserves1[0] / fee;
    vec_h[(0, 0)] = reserves1[0] * reserves1[1] * 0.0001; // Allow for 0.01% slippage

    mat_g[(1, 2)] = reserves2[0];
    mat_g[(1, 3)] = -reserves2[1] / fee;
    vec_h[(1, 0)] = reserves2[0] * reserves2[1] * 0.0001; // Allow for 0.01% slippage
